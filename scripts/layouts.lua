Tracker:AddLayouts("layouts/frogs.jsonc")
Tracker:AddLayouts("layouts/horizontal_collectibles.jsonc")
Tracker:AddLayouts("layouts/horizontal_dungeons.jsonc")
Tracker:AddLayouts("layouts/horizontal_equipment.jsonc")
Tracker:AddLayouts("layouts/horizontal_items.jsonc")
Tracker:AddLayouts("layouts/horizontal_masks.jsonc")
--Tracker:AddLayouts("layouts/horizontal_remnants.jsonc")
Tracker:AddLayouts("layouts/horizontal_trades.jsonc")
--Tracker:AddLayouts("layouts/logic_tricks_tab.jsonc")
Tracker:AddLayouts("layouts/owl_statues.jsonc")
Tracker:AddLayouts("layouts/scarecrows.jsonc")
Tracker:AddLayouts("layouts/settings_goals.jsonc")
Tracker:AddLayouts("layouts/settings_other.jsonc")
Tracker:AddLayouts("layouts/settings_popup.jsonc")
Tracker:AddLayouts("layouts/settings_sanities.jsonc")
Tracker:AddLayouts("layouts/settings_shuffles.jsonc")
Tracker:AddLayouts("layouts/settings_souls.jsonc")
Tracker:AddLayouts("layouts/souls_absurd.jsonc")
Tracker:AddLayouts("layouts/souls_enemies.jsonc")
Tracker:AddLayouts("layouts/souls_boss.jsonc")
Tracker:AddLayouts("layouts/souls_misc.jsonc")
Tracker:AddLayouts("layouts/souls_npc.jsonc")
Tracker:AddLayouts("layouts/tracker.jsonc")
Tracker:AddLayouts("layouts/vertical_collectibles.jsonc")
Tracker:AddLayouts("layouts/vertical_dungeons.jsonc")
Tracker:AddLayouts("layouts/vertical_equipment.jsonc")
Tracker:AddLayouts("layouts/vertical_items.jsonc")
Tracker:AddLayouts("layouts/vertical_masks.jsonc")
