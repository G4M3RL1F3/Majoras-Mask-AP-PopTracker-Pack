...
function clear_woodfall()
    return woodfall_temple() and has("bow") and can_smack() and has("soul_odolwa") and (has("bosskey_wf") or (has("odolwa") and has("remains_allow_boss_warps")))
end
...
function clear_snowhead()
    return snowhead_temple() and can_use_fire_arrows() and has("soul_goht") and ((has("smallkey_sh", 1) and has("bosskey_sh")) or (has("goht") and has("remains_allow_boss_warps")))
end
